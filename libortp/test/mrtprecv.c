...
static char *help = "usage: mrtprecv file_prefix local_port number_of_streams,"
                    "Receives multiples rtp streams on local_port+2*k, "
                    "k={0..number_of_streams}\n";
...
#ifndef _WIN32
        filefd[i] = open(filename, O_WRONLY | O_CREAT | O_TRUNC,
                S_IRUSR|S_IWUSR|S_IRGRP|S_IWGRP);
#else
        filefd[i]=open(filename, _O_BINARY | O_WRONLY | O_CREAT | O_TRUNC);
#endif
        if (filefd[i] < 0)
            ortp_error("Could not open %s for writing: %s",
